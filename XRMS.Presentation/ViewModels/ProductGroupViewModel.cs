...
{
    public class ProductGroupViewModel : ItemViewModelBase<ProductGroup>
    {
        ...
        public ProductGroupViewModel(IMessageBoxService messageBoxService, ProductGroup item, IProductGroupManager manager) : base(messageBoxService, item, manager)
        {
            // do initialization
            try
            {
                this.DisplayName = "Edit Product Group: " + this.Item.Name;
            }
            catch (Exception ex)
            {
                this.MessageBoxService.ShowError(this.GetType().FullName + System.Reflection.MethodBase.GetCurrentMethod().Name + ": " + ex.Message);
